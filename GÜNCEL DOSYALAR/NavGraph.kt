...
import com.cyberqbit.ceptekabin.ui.screens.dolap.KiyaketDetayScreen
...
/** Ana ekran route'ları — bu ekranlardayken pending import işlenir */
...
fun NavGraph(
    ...
    /**
     * MainActivity'den gelen .kmb URI'si.
     * null ise bekleyen bir import yok demektir.
     */
    pendingImportUri: Uri? = null,
    /**
     * URI işlendikten sonra MainActivity'yi bilgilendirir,
     * böylece aynı URI tekrar işlenmez.
     */
    ...
    // ── Deep Link: pending .kmb import ────────────────────────────────────────
    // Kullanıcı ana ekranlardayken (Splash/Auth geçtikten sonra) import sayfasını aç.
    LaunchedEffect(pendingImportUri, currentRoute) {
        val uri = pendingImportUri ?: return@LaunchedEffect
        // Sadece ana ekranlarda tetikle (kimlik doğrulama tamamlanmış olmalı)
        if (currentRoute in mainScreenRoutes) {
            val encoded = Uri.encode(uri.toString())
            navController.navigate(Screen.KombinImport.createRoute(encoded)) {
    ...
    Scaffold(
        ...
        NavHost(
            ...
            // ── Kombin Import (Deep Link) ─────────────────────────────────────
            composable(
                route     = Screen.KombinImport.route,
                ...
                KombinImportScreen(
                    ...
                    onImportSuccess = { kombinId ->
                        // İmport başarılı → doğrudan kombinin detay sayfasına git
                        navController.navigate(Screen.KombinDetay.createRoute(kombinId)) {
                            popUpTo(Screen.KombinImport.route) { inclusive = true }
                            launchSingleTop = true
                        }
                    }
                )
